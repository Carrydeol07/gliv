...
# 004_ALL_COMPONENTS.md

> Version: 2.0 Status: Locked

  ---------------------------------------
  \## Table of Contents -
  23_SERIES_CARD_SPECIFICATION -
  24_SEARCH_SYSTEM_SPECIFICATION -
  25_SERIES_PAGE_SPECIFICATION -
  26_LIBRARY_SPECIFICATION -
  27_COLLECTIONS_SPECIFICATION -
  28_DISCOVER_SPECIFICATION -
  29_UPDATES_SPECIFICATION -
  30_IMPORT_SYSTEM_SPECIFICATION -
  31_PROVIDER_MANAGER_SPECIFICATION -
  33_UI_DESIGN_SYSTEM_SPECIFICATION -
  35_AUTHOR_PAGE_SPECIFICATION -
  36_ARTIST_PAGE_SPECIFICATION -
  37_SETTINGS_SPECIFICATION -
  38_SIDEBAR_SPECIFICATION -
  39_SEARCH_RESULT_CARD_SPECIFICATION -
  40_UPDATE_CARD_SPECIFICATION -
  41_COLLECTION_CARD_SPECIFICATION -
  42_PROGRESS_WIDGET_SPECIFICATION -
  47_BACKUP_SYSTEM_SPECIFICATION -
  48_EDIT_HISTORY_SPECIFICATION
...
> Version: 2.0 Status: Locked Component
...
Each displayed contributor is clickable and opens the appropriate
Contributor page.

Only the primary contributor is shown on the Series Card to preserve
readability.
...
The Series Card intentionally prioritizes the user's personal library
information over external metadata.
...
The Search System should answer most questions without requiring users
...
Provider Manager automatically routes requests to the appropriate
provider based on media type.
...
If no suitable provider-backed Title exists, users may create a Manual
Title.
...
-   Manual Titles are offered only when no suitable provider-backed
    Title exists.
...
The Series Page prioritizes personal information while presenting
provider data as supporting information.
...
The Header should immediately identify the Title while exposing the
user's personal relationship with it.
...
The interface should clearly distinguish Manual Titles from
provider-backed Titles without reducing functionality.
...
Contributor relationships are accessed through Contributor pages rather
than appearing as connection types.
...
It provides fast access to every Title in the user's collection while
preserving the user's personal organization and progress.
...
Collections allow users to organize their Library beyond standard
statuses.

Collections are entirely personal and never modified by provider
synchronization.
...
Grouping is purely visual and does not create Connections between
Titles.
...
Opening a Collection displays Titles using the shared Series Card
component.
...
Discover helps users find new Titles and add them to their personal
Library.

It combines Universal Search with curated discovery features while
keeping the interface simple and content-focused.
...
Universal Search searches across all supported media types through the
Provider Manager.
...
When Universal Search cannot find a suitable provider-backed Title,
...
-   Do not display provider-only features such as Availability or Live
    Updates.
...
The Updates view presents meaningful changes for Titles already in the
user's Library.

It helps users stay informed about new content without overwhelming them
...
When provider information is unavailable, previously synchronized
...
The Import Engine brings external library data into GLIV while
preserving personal information and preventing incorrect provider
matches.

Every import follows the same validation and review pipeline regardless
of its source.
...
Provider matching attempts to identify the imported Title using provider
metadata.
...
If no suitable provider-backed match exists, users may create a Manual
Title directly from Import Review.

Manual Titles preserve all imported personal information but do not
receive provider synchronization.
...
The Provider Manager is the single gateway between GLIV and every
...
If no suitable provider-backed Title exists, the Provider Manager allows
the workflow to continue through Manual Title creation.
...
When multiple providers return equivalent Titles, the Provider Manager
merges provider information into a single result before presenting it to
the user.

Duplicate resolution never merges personal Library entries
automatically.
...
The application should always remain responsive while provider requests
...
-   Primary → Cache → Secondary → Graceful Failure is consistently
    applied.
-   Duplicate provider results are merged before presentation.
-   Manual Titles integrate seamlessly when no provider-backed Title
    exists.
...
This specification defines how shared UI components are implemented
consistently throughout GLIV.

It complements the Design System by translating visual guidelines into
...
All screens should reuse these components rather than creating custom
implementations.
...
The same interaction should always behave the same way throughout the
application.
...
Accessibility is a core implementation requirement rather than an
...
-   New screens reuse existing components rather than introducing
...
The Author Page presents information about a Contributor whose role
includes Author.

It allows users to explore the Contributor's biography, Titles in their
Library, and other published works.
...
A Contributor may have multiple roles, but this page is presented in the
context of the Author role.
...
Displays additional provider-backed Titles associated with the
Contributor.

Titles not already in the Library may be opened or added through the
...
-   Provider information remains clearly separated from personal
    information.
...
The Artist Page presents information about a Contributor whose role
includes Artist.

It allows users to explore the Contributor's portfolio, Titles in their
Library, and other illustrated works.
...
Displays additional provider-backed Titles illustrated by the
Contributor.
...
Displays notable collaborations between the Artist and other
Contributors when provider information is available.
...
-   Provider information remains clearly separated from personal
    information.
-   Collaborations provide additional discovery without distracting from
...
-   Library Titles appear before external works.
-   Contributor information remains easy to understand.
-   Collaborations are clearly distinguished from the Contributor's own
    works.
-   Navigation between Contributors and Series feels seamless.
...
The Settings page allows users to configure GLIV without exposing
unnecessary complexity.

Frequently used settings should be easy to find, while advanced options
remain available without overwhelming the interface.
...
Provider selection remains automatic where defined by the Provider
Manager.
...
It remains visible across the application and allows users to quickly
move between the application's primary sections.
...
-   The Sidebar behaves consistently in both collapsed and expanded
    modes.
...
It provides enough information for users to identify a Title and decide
whether to view it, add it to their Library, or create a Manual Title
when no suitable provider-backed result exists.
...
The card clearly indicates whether the Title already exists in the
user's Library.
...
-   Manual Title creation is offered only when no suitable
    provider-backed result exists.
...
The Update Card represents a single update for a Title already in the
user's Library.

It provides a concise summary of what changed and allows users to
quickly open the affected Series.
...
The Collection Card represents a single Collection within the
Collections view.

It provides a concise summary of the Collection while allowing quick
navigation to its contents.
...
The cover collage is automatically generated from Titles contained
...
-   Empty Collections should encourage adding Titles rather than
    appearing broken.
-   Collection Cards should remain visually consistent throughout the
    application.
...
The Progress Widget displays and manages personal progress for a single
Format.

It provides a consistent interface for viewing progress, updating
progress, and managing Progress Overrides for provider-backed Formats.
...
Effective Latest is calculated by the business rules and represents the
highest available progress after applying any active Progress Override.
...
Removing an Override immediately restores the provider-calculated
Effective Latest.

When the provider catches up to the Override value, the Override is
...
-   The widget displays calculated values but does not implement
    business logic.
...
The Backup System protects the user's personal library and application
data against accidental loss, corruption, or migration failures.
...
Before restoration begins, GLIV automatically creates a new safety
...
-   Restoring a backup never permanently destroys the current database
    without first creating a safety backup.
