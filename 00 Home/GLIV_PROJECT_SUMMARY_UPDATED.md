...
- **Module 05 — Cache System** — Merged. Split-tier cache (ADR-018) implemented for Provider Manager, segregating Discover searches (Tier 1) from Library updates (Tier 2).
- **Module 06 — Search System** — Merged. Universal Search orchestration implemented (combining local SQLite text-match with Provider Manager results). Resolves duplicates via `external_references` cross-check. `ALREADY_PLANNED` state detection is deferred until `planned_titles` is built by a future Collections module (per ADR-019). **`24_SEARCH_SYSTEM_SPECIFICATION.md` received a v2.1 amendment:** Provider results now rank *above* Library results in Discover scope (Discover exists to find new things, not re-find owned things). Library matches are appended as a block below, appropriately flagged.
- **Module 07 — Library** — Merged. Library page and Series Card implemented with robust sorting and filtering. Implementation surfaced a gap where Rating, Favorite, and Original Order were completely missing from the database specification. Patched via an architectural decision to attach these single-valued personal attributes directly to the `titles` table.
- **Module 08 — Series Page** — Merged. Full Series Page implemented: Progress Widget, Progress Override dialog, Availability/Publication/Connections/Contributor panels, Synopsis Panel, Personal Notes, Collection toggle, and "Add Another Format" (Search → Import Review) entry point. Surfaced a real gap in BR-004 (no rule distinguished a PENDING/USER_REJECTED external_reference from a confirmed one) — patched as BR-004 v1.1, adding the "Manual Format Determination" section: only AUTO/USER_CONFIRMED counts as provider-backed.
